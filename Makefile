...
ifeq ($(wildcard $(ENV_OPENSSL_PREFIX)/lib),)
	ifeq ($(wildcard $(ENV_OPENSSL_PREFIX)/lib64),)
		OPENSSL_LIB_DIR := ENV_OPENSSL_PREFIX
	endif
	OPENSSL_LIB_DIR := $(addprefix $(ENV_OPENSSL_PREFIX), /lib64)
else
	OPENSSL_LIB_DIR := $(addprefix $(ENV_OPENSSL_PREFIX), /lib)
endif
...
openssl3:
	chmod +x ./utils/install-openssl.sh
